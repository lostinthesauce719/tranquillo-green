...
// Generate a downloadable CSV export of the packet
function generatePacketExport({
  bundle,
  formats,
  schedules,
  coverMemoMode,
  ...
  // Header
  lines.push("TRANQUILLO GREEN — CPA HANDOFF PACKET");
  lines.push(`Generated: ${new Date().toISOString()}`);
  lines.push(`Bundle: ${bundle.name}`);
  lines.push(`Recipient: ${bundle.recipient}`);
  lines.push(`Owner: ${bundle.owner}`);
  lines.push(`Framing: ${coverMemoMode.replaceAll("_", " ")}`);
  lines.push("");

  // Formats
  lines.push("EXPORT FORMATS");
  formats.forEach((f) => lines.push(`  - ${f.toUpperCase()}`));
  lines.push("");

  // Included schedules
  lines.push("INCLUDED SCHEDULES");
  schedules.forEach((s) => lines.push(`  - ${s}`));
  lines.push("");

  // Cover memo
  lines.push("COVER MEMO");
  if (coverMemoMode === "controller_summary") {
    lines.push("This packet provides a controller summary of the period-end close.");
    lines.push("Key areas: 280E allocation splits, cash reconciliation variances,");
    lines.push("and compliance filing status. All items reviewed and signed off.");
  } else if (coverMemoMode === "cpa_handoff") {
    lines.push("This packet is prepared for CPA review and external filing support.");
    lines.push("Includes 280E support schedules, allocation override history,");
    lines.push("and all supporting documentation for tax provision work.");
  } else {
    lines.push("OPEN ITEMS LIST");
    lines.push("The following items require resolution before close can be finalized:");
    checklist.filter((c) => c.status !== "done").forEach((c) => lines.push(`  - [${c.status.toUpperCase()}] ${c.title} (${c.owner})`));
  }
  lines.push("");

  // Checklist
  lines.push("ATTACHED CHECKLIST");
  checklist.forEach((c) => {
  ...
  if (bundle.blockers.length > 0) {
    lines.push("  Blockers:");
    bundle.blockers.forEach((b) => lines.push(`    - ${b}`));
  } else {
    lines.push("  No blockers.");
  }
  lines.push("");

  if (includeDeliveryNotes) {
    lines.push("DELIVERY NOTES");
    lines.push("  Packet prepared for secure delivery to CPA.");
    lines.push("  Review all items before signing off on release.");
    lines.push("");
  }

  lines.push("---");
  lines.push("Generated by Tranquillo Green · Cannabis Accounting OS");
