...
export default defineSchema({
  ...
  exportPacketRuns: defineTable({
    ...
    recipient: v.string(),
    owner: v.string(),
    status: v.union(v.literal("draft"), v.literal("generated"), v.literal("sent"), v.literal("held")),
    selectedFormats: v.array(v.string()),
    selectedSchedules: v.array(v.string()),
    selectedChecklistTitles: v.array(v.string()),
    coverMemoMode: v.union(v.literal("controller_summary"), v.literal("cpa_handoff"), v.literal("open_items")),
    includeDeliveryNotes: v.boolean(),
    generatedAt: v.number(),
    generatedBy: v.string(),
    detail: v.string(),
    blockers: v.array(v.string()),
  }).index("by_company", ["companyId"]).index("by_company_period", ["companyId", "periodLabel"]),
  ...
  accountingAuditEvents: defineTable({
    ...
    category: v.union(v.literal("reporting_period"), v.literal("reconciliation"), v.literal("export_packet"), v.literal("allocation_override")),
    entityId: v.string(),
    entityLabel: v.string(),
    action: v.string(),
    detail: v.string(),
    actor: v.string(),
    source: v.union(v.literal("user"), v.literal("system"), v.literal("server_action")),
    occurredAt: v.number(),
  }).index("by_company", ["companyId"]).index("by_company_category", ["companyId", "category"]),
});
