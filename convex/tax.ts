import { authenticate, authMutation, authQuery, requireCompanyAccess } from "./lib/withAuth";
import { db, query, update } from "@/convex/server";
...
export const listTaxTypes = authQuery({
  args: {},
  handler: async (ctx) => {
    return await db.query("taxTypes").collect();
  },
});
...
export const calculateTax = authMutation({
  ...
  handler: async (ctx, payload) => {
    ...
    const jurisdiction = await db.get(targetJurisdiction);
    ...
    for (const typeId of typesToCalculate) {
      const taxType = await db.get(typeId);
      if (!taxType) continue;
      ...
      const calcId = await db.insert("taxCalculations", {
        companyId,
...
export const getTaxLiability = authQuery({
  ...
  handler: async (ctx, { companyId, periodStart, periodEnd }) => {
    ...
    for (const calc of calculations) {
      const jurisdiction = await db.get(calc.jurisdictionId);
      const taxType = await db.get(calc.taxTypeId);
...
export const upsertTaxJurisdiction = authMutation({
  ...
  handler: async (ctx, { jurisdictionId, data }) => {
    ...
    if (jurisdictionId) {
      await db.update(jurisdictionId, { ...data, updatedAt: Date.now() });
      return { success: true, jurisdictionId };
    } else {
      const id = await db.insert("taxJurisdictions", {
        ...data,
...
export const upsertTaxRate = authMutation({
  ...
  handler: async (ctx, { rateId, data }) => {
    if (rateId) {
      await db.update(rateId, { ...data });
      return { success: true, rateId };
    } else {
      const id = await db.insert("taxRates", data);
...
export const updateCompanyTaxProfile = authMutation({
  args: {
    ...
    filingCalendar: v.record(v.string()),
  ...
  handler: async (ctx, { companyId, primaryJurisdictionId, nexusStates, filingCalendar, taxTypesEnabled }) => {
    ...
    if (existing) {
      ...
      const id = await db.insert("taxProfiles", {
        companyId,
